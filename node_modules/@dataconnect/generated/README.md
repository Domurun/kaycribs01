...
- [**Queries**](#queries)
  ...
  - [*GetMySavedProperties*](#getmysavedproperties)
- [**Mutations**](#mutations)
...
## GetProperties
You can execute the `GetProperties` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
getProperties(): QueryPromise<GetPropertiesData, undefined>;
...
```typescript
getProperties(dc: DataConnect): QueryPromise<GetPropertiesData, undefined>;
...
The `data` property is an object of type `GetPropertiesData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getProperties } from '@dataconnect/generated';
...
import { connectorConfig, getPropertiesRef } from '@dataconnect/generated';
...
You can execute the `GetMySavedProperties` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
getMySavedProperties(): QueryPromise<GetMySavedPropertiesData, undefined>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `QueryRef` function.
```typescript
getMySavedProperties(dc: DataConnect): QueryPromise<GetMySavedPropertiesData, undefined>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the getMySavedPropertiesRef:
```typescript
const name = getMySavedPropertiesRef.operationName;
console.log(name);
```

### Variables
The `GetMySavedProperties` query has no variables.
### Return Type
Recall that executing the `GetMySavedProperties` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `GetMySavedPropertiesData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `GetMySavedProperties`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getMySavedProperties } from '@dataconnect/generated';


// Call the `getMySavedProperties()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getMySavedProperties();

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await getMySavedProperties(dataConnect);

console.log(data.savedProperties);

// Or, you can use the `Promise` API.
getMySavedProperties().then((response) => {
  const data = response.data;
  console.log(data.savedProperties);
});
```

### Using `GetMySavedProperties`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, getMySavedPropertiesRef } from '@dataconnect/generated';


// Call the `getMySavedPropertiesRef()` function to get a reference to the query.
const ref = getMySavedPropertiesRef();

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getMySavedPropertiesRef(dataConnect);
...
const { data } = await createUser({ email: ..., userType: ..., });
...
// Variables can be defined inline as well.
const ref = createUserRef({ email: ..., userType: ..., });
...
// Variables can be defined inline as well.
const { data } = await createInquiry({ propertyId: ..., message: ..., toAgentId: ..., });
...
// Variables can be defined inline as well.
const ref = createInquiryRef({ propertyId: ..., message: ..., toAgentId: ..., });
